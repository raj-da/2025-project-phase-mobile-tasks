...
class ProductBloc extends Bloc<ProductEvent, ProductState> {
  ...
  ProductBloc({
    ...
    on<ToUpdateEvent>(_onToUpdate);
    on<ToCreateEvent>(_onToCreate);
  }
  ...
  void _onLoadAllProducts(
    ...
    failureOrProducts.fold(
      (failure) =>
          emit(ProductError('Failed to load products: ${failure.toString()}')),
  ...
  void _onGetSingleProduct(
    ...
    failureOrProduct.fold(
      (failure) =>
          emit(ProductError('Failed to load product: ${failure.toString()}')),
      (product) => emit(
        LoadedSingleProductState(product!),
      ), // Assuming product is never null on success
    );
  }
  ...
  void _onCreateProduct(
    ...
    failureOrSuccess.fold(
      (failure) =>
          emit(ProductError('Failed to create product: ${failure.toString()}')),
      (_) {
        // emit(ProductSuccess());
  ...
  void _onUpdateProduct(
    ...
    failureOrSuccess.fold(
      (failure) =>
          emit(ProductError('Failed to update product: ${failure.toString()}')),
...
void _onToUpdate(ToUpdateEvent event, Emitter<ProductState> emit) {
  emit(ToUpdateState(event.product));
}

void _onToCreate(ToCreateEvent event, Emitter<ProductState> emit) {
  emit(ToCreateState());
}
